...
use forge::{LocalNode, NodeExt, SwarmExt};
...
fn update_node_config(validator: &mut LocalNode, mut config: NodeConfig) {
    validator.stop();
...
async fn test_genesis_transaction_flow() {
    ...
    env.wait_for_all_nodes_to_catchup(Duration::from_secs(5))
        .await
        .unwrap();
    ...
    node.start().unwrap();
    check_create_mint_transfer_node(&mut env, 1).await;
}
